...
@io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "2xx", description = "Response thành công", content = @Content(mediaType = MediaType.APPLICATION_JSON_VALUE, schema = @Schema(implementation = SuccessResponse.class)))
@io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "4xx, 5xx", description = "Response về danh sách lỗi, nếu chỉ có 1 lỗi thì field đầu tiên sẽ có giá trị \"general\". Còn lại là tên của filed lỗi", content = @Content(mediaType = MediaType.APPLICATION_JSON_VALUE, schema = @Schema(implementation = ErrorResponse.class)))
...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ErrorResponse> handlerAccessDeniedException(AccessDeniedException e, HttpServletRequest request) {
        return ApiResponse.createErrorResponse(HttpStatus.FORBIDDEN, "You do not have permission to access this resource: " + request.getServletPath());
    }
