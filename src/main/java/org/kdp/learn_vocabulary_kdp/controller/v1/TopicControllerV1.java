 ...
 * @LastModified: 2024/12/20 - 00:42 AM (ICT)
 ************************************************/
...
public class TopicControllerV1 {
    ...
    public ResponseEntity<Object> getWords(@PathVariable String topicId) {
        return ApiResponse.createSuccessResponse(HttpStatus.OK, GlobalMessage.SUCCESSFULLY, topicService.getWords(topicId));
    }

    @PostMapping("/{topicId}/words")
    public ResponseEntity<Object> addWord(@PathVariable String topicId, @Valid @RequestBody WordCreationRequest request) {
        return ApiResponse.createSuccessResponse(HttpStatus.CREATED, GlobalMessage.SUCCESSFULLY, topicService.addWord(topicId, request));
    }

    @PutMapping("/{topicId}/words/{wordId}")
    public ResponseEntity<Object> updateWord(@PathVariable String topicId, @PathVariable String wordId, @RequestBody WordUpdateRequest request) {
        return ApiResponse.createSuccessResponse(HttpStatus.OK, GlobalMessage.SUCCESSFULLY, topicService.updateWord(topicId, wordId, request));
    }
